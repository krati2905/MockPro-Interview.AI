...
def _mock_questions(resume_text, domain=None):
    ...
    # Always include some behavioral and HR questions
    common_questions = [
    ...
    # Combine domain-specific and common questions
    all_questions = domain_specific + common_questions[:2]  # Ensure we have exactly 6
...
def generate_questions(resume_text, domain=None):
    """Generate interview questions using Google AI Studio Gemini when available, otherwise fallback logic."""

    domain_prompt = f" Focus on {domain.replace('_', ' ')} domain." if domain else ""

    payload = _call_gemini(
        (
            f"You are an interview coach. Generate exactly 6 interview questions based on the resume below. "
            "Use categories technical, hr, and behavioral. "
            "Return strict JSON only in this format: {\"questions\": [{\"category\": \"technical\", \"question\": \"...\"}]}.\n\n"
            f"Resume:\n{(resume_text or '')[:6000]}{domain_prompt}"
        ),
        temperature=0.6,
    )
...
def evaluate_answer(question, answer):
    ...
    payload = _call_gemini(
        (
            ...
            "score, strengths, weaknesses, suggested_answer. Score must be between 0 and 10.\n\n"
            f"Question: {question}\nAnswer: {answer}"
        ),
        temperature=0.4,
    )
